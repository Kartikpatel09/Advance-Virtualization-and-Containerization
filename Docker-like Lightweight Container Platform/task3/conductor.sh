...
handle_run() {
    ...
    mkdir -p "$CACHEDIR/layers/$layer_hash/rootfs"

    # Ensure parent_layers is valid
    # [ -z "$parent_layers" ] && parent_layers="/dev/null"
    # echo "Mounting overlay with:"
    # echo "  lowerdir=$parent_layers"
    # echo "  upperdir=$CACHEDIR/layers/$layer_hash/diff"
    # echo "  workdir=$CACHEDIR/layers/$layer_hash/work"
    # echo "mergeddir=$CACHEDIR/layers/$layer_hash/rootfs"
    # Subtask 3.f.2
    ...
    mount -t overlay overlay \
        ...
        "$CACHEDIR/layers/$layer_hash/rootfs" || die "Mount failed in Handle Run"

    # Subtask 3.f.3
    # Execute the command in the new mount
    chroot "$CACHEDIR/layers/$layer_hash/rootfs" bash -c "$command"


    # Unmount the overlay filesystem
    umount "$CACHEDIR/layers/$layer_hash/rootfs" || die "Unmount failed in Handle Run"
...
handle_copy() {
    ...
    local dest_hash=$(echo "$dest" | sha256sum | cut -d' ' -f1)
    local content_hash=$(find "$src" -type f -exec sha256sum {} + | sha256sum | cut -d' ' -f1)
    local layer_hash=$(echo "COPY-${parent_hash}-${dest_hash}-${content_hash}" | sha256sum | cut -d' ' -f1)
    ...
    # Debug: Print mount command before execution for testing purpose
    # echo "Mounting overlay with:"
    # echo "  lowerdir=$parent_layers"
    # echo "  upperdir=$CACHEDIR/layers/$layer_hash/diff"
    # echo "  workdir=$CACHEDIR/layers/$layer_hash/work"
    ...
    mount -t overlay overlay \
        -o lowerdir="$parent_layers",upperdir="$CACHEDIR/layers/$layer_hash/diff",workdir="$CACHEDIR/layers/$layer_hash/work" \
        "$CACHEDIR/layers/$layer_hash/rootfs" || die "Mount failed in Handle Copy"

    # Subtask 3.e.3
    # Copy the files from source to destination
    cp -r "$src" "$CACHEDIR/layers/$layer_hash/rootfs/$dest"

    # Subtask 3.e.4
    # Unmount the overlay filesystem
    umount "$CACHEDIR/layers/$layer_hash/rootfs" || die "Unmount failed in Handle Copy"
...
run() {
    ...
    local LAYER=$(cat "$IMAGEDIR/$IMAGE/layers") || die "Error in reading layer in run "

    # Read the layer stack from the image directory and mount the overlay filesystem
    # Ensure only one overlayfs is mounted
    if ! mountpoint -q "$CONTAINERDIR/$NAME/rootfs"; then
        mount -t overlay overlay -o lowerdir="$LAYER",upperdir="$CONTAINERDIR/$NAME/upper",workdir="$CONTAINERDIR/$NAME/work" "$CONTAINERDIR/$NAME/rootfs"
    else
        echo "Overlay filesystem is already mounted for container $NAME"
    fi
    # Subtask 3.d.2 - end
    ...
    mkdir -p "$CONTAINERDIR/$NAME/rootfs/dev"
    mount --bind /dev "$CONTAINERDIR/$NAME/rootfs/dev"
    # Subtask 3.d.3
    # Modify subtask 3.a.1 to bind mount /dev
    # Subtask 3.a.2
    # - Use unshare to run the container in a new [uts(u), pid(p), net(n), mount(m), ipc(i)] namespaces
   ...
   unshare -u -m -p -f -i --n --kill-child \
...
stop() {
    ...
    # Subtask 3.d.4
    # Unmount the overlay filesystem
    umount "$CONTAINERDIR/$NAME/rootfs"
...
exec() {
    ...
    nsenter --target "$CONTAINER_INIT_PID" -p -u -n -m -w -i -r $EXEC_CMD_ARGS
...
addnetwork() {
    ...
    ip link add "$OUTSIDE_PEER" type veth peer name "$INSIDE_PEER" netns "$NAME"
    ...
    # Enable outside interface from the host
    ip link set "$OUTSIDE_PEER" up
    nsenter --target "$PID" -p -u -n -m -w -i -r /bin/bash -c 'ip link set "$0" up &&
    ip link set lo up &&
    exit' "$INSIDE_PEER"
