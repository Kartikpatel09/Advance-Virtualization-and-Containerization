		...
		case KVM_EXIT_IO:
			if (vcpu->kvm_run->io.direction ==KVM_EXIT_IO_OUT)
			{
				...
				else if (vcpu->kvm_run->io.port == 0xE8)
				{
					...
					uint32_t *input = (uint32_t *)((char *)vm->mem + guest_address);
					printf("%u\n", *input);
					continue;
				}
				else if (vcpu->kvm_run->io.port == 0xE6)
				{
					io_out++;
					void *data_ptr = ((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
					memcpy(&guest_address, data_ptr, sizeof(uint32_t));
					char *input = ((char *)vm->mem + guest_address);
					printf("%s", input);
					continue;
				}
				...
				{	io_out++;
					...
					memcpy(&guest_address, (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset, sizeof(uint32_t));

					// Access guest memory at the provided address
					m = (char *)vm->mem + guest_address;
					
					continue;
				}
				else if (vcpu->kvm_run->io.port == 0xE4)
				{
					io_out++;
					struct kvm_translation translate;
					uint32_t gva;
					memcpy(&gva, (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset, sizeof(uint32_t));
					memset(&translate, 0, sizeof(translate));
					translate.linear_address = (uint64_t)gva;

					if (ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &translate) < 0)
					{
					...
					// Return lower 32 bits of HVA
					hva = (uintptr_t)vm->mem + translate.physical_address;
					continue;
				}
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN)
			{
				...
				{
					io_in++;
					int32_t *input_buffer = (int32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
					*input_buffer = hypercall_count;
					continue;
				}
				else if (vcpu->kvm_run->io.port == 0xE3)
				{
					io_in++;
				...
				else if(vcpu->kvm_run->io.port == 0xE2){
					io_in++;
					int32_t *input_buffer = (int32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
					*input_buffer = 1;
					snprintf(m, 50, "IO in: %d\nIO:out: %d\n", io_in, io_out);
					continue;
				}
			}
			/* fall through */
