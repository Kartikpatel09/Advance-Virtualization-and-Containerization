	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xE9)
			{
				...
				int *guest_memory = (int *)((char *)vm->mem + guest_address);
				printf("VMFD: %d Produced Values: ", vm->vm_fd);
				for (int i = 0; i < 5; i++)
				{
					printf("%d ", guest_memory[i]);
					arr[i] = guest_memory[i];
				}
				printf("\n");
				vm = vm2;
				vcpu = vcpu2;
			...
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xE7)
			{
				memcpy(&guest_address, (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset, sizeof(uint32_t));
				sharedData *a = (sharedData *)((char *)vm->mem + guest_address);
				a->count=5;
				for (int i = 0; i < 5; i++)
				{
					a->buff[i] = arr[i];
			...
			{
				memcpy(&guest_address, (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset, sizeof(uint32_t));
				sharedData *b = (sharedData *)((char *)vm->mem + guest_address);
