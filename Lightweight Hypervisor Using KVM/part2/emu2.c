...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    while (1)
    {
        ...
        case KVM_EXIT_IO:
            ...
            {
                val[i] = *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset);
            ...
            if (vm->vcpus->kvm_run->io.direction == KVM_EXIT_IO_IN && vm->vcpus->kvm_run->io.port == 0x11)
            {
                input = (int *)((char *)vm->vcpus->kvm_run + vm->vcpus->kvm_run->io.data_offset);
                *input = val[j];
                break;
            }
            if (vm->vcpus->kvm_run->io.direction == KVM_EXIT_IO_OUT && vm->vcpus->kvm_run->io.port == 0x12)
            {
                int k = *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset);
